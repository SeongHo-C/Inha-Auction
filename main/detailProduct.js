...
const productId = url.substring(url.length - 1, url.length);
  ...
  .then(function (data) {
    ...
    function imageAdd() {
      ...
      htmlData +=
        '<img src="http://182.218.194.156:8080/image/product-' +
        productId +
        '-1.png?type=product" class="d-block w-100 detail-image-size" alt="..."/></div>';

      htmlData += '<div class="carousel-item">';
      htmlData +=
        '<img src="http://182.218.194.156:8080/image/product-' +
        productId +
        '-2.png?type=product" class="d-block w-100 detail-image-size" alt="..."/></div>';
    ...
    function productAdd() {
      ...
      const product = data.data;
      htmlData +=
        '<thead><tr><th>시작가</th><td style="color: blue">' +
        product.startPrice +
        '원</td>';
      htmlData += '<td id="time"></td></tr></thead>';
      htmlData += '<tbody><tr><th>상품번호</th><td>' + product.id + '</td>';
      htmlData +=
        '<td><a href="/main/notify.html"><button type="button">신고</button></a></td></tr>';
      htmlData += '<tr><th>마감기한</th><td> ' + product.endDate + '</td></tr>';
      htmlData += '<tr><th>입찰단위</th><td>' + product.bidUnit + '</td></tr>';
      htmlData +=
        '<tr><th>즉시구매가</th><td>' + product.instantPrice + '</td></tr>';
    ...
    function contentAdd() {
      ...
      const product = data.data;
      htmlData +=
        '<h2>상품설명</h2></hr><textarea style="width: 100%; height: auto" readonly>' +
        product.content +
        '</textarea>';
